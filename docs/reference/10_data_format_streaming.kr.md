...
```csv
# imu_data.csv
# timestamp_ns: 나노초, sensor_timestamp 기준
...
Header (32 bytes):
  ...
  - Start timestamp: 8 bytes (int64, 나노초)
  - IMU rate: 4 bytes (float32, Hz)
  - Reserved: 12 bytes

Each Record (40 bytes):
  - Timestamp: 8 bytes (int64, 나노초)
...
message SynchronizedFrame {
    ...
    // 프레임 전후의 IMU 데이터
    repeated ImuSample imu_before = 2;  // 이전 프레임 ~ 현재 프레임
    repeated ImuSample imu_after = 3;   // 현재 프레임 ~ 다음 프레임 (선택)

    // 이미지 데이터 (옵션: 별도 파일 참조 가능)
    bytes image_data = 4;
    string image_path = 5;  // 또는 외부 파일 경로
}
...
message RecordingSession {
    ...
    message Transform {
        repeated double rotation = 1;     // 쿼터니언 [w, x, y, z]
        repeated double translation = 2;  // [x, y, z] meters
    }
    Transform T_imu_camera = 6;

    double time_offset_imu_camera_s = 7;  // IMU - Camera 시간 오프셋 (초)
}
```
...
class UdpImuStreamer(
    ...
    fun sendImuSample(
        ...
        // Sequence number (4 bytes) - 패킷 손실 감지용
...
### 2.2 WebRTC 기반 비디오 스트리밍

#### Android WebRTC 설정 (Kotlin)
...
class IntegratedStreamReceiver:
    """
    비디오(WebRTC)와 IMU(UDP)를 통합 수신하는 클라이언트
    """
    ...
    def __init__(self, video_port: int = 8080, imu_port: int = 5005):
        ...
        self.imu_buffer = deque(maxlen=50000)  # ~250초 @ 200Hz
        self.frame_buffer = deque(maxlen=100)   # ~3초 @ 30fps
...
| 포맷 | 용량 | 파싱 속도 | 호환성 | 스키마 | 권장 용도 |
|------|------|----------|--------|--------|----------|
| CSV | 높음 | 느림 | 최고 | 없음 | 디버깅, 소규모 |
| Binary | 최저 | 최고 | 낮음 | 고정 | 실시간 스트리밍 |
| Protobuf | 낮음 | 빠름 | 중간 | 있음 | 프로덕션, 대규모 |
| JSON | 매우 높음 | 느림 | 높음 | 유연 | 설정, 메타데이터 |
